...
*Note: the full dataset as well as all QMD files are stored in this public [GitHub repo](https://github.com/natekrall24/STA465FinalProject/tree/main).*
...
#### Case study: Taylor Swift vs. Drake

As a focused application of our PCA framework, we compare Taylor Swift and Drake, two highly popular artists with distinct musical styles. Using PCA scores from the standardized audio features, we project tracks by each artist into a shared low-dimensional space to assess whether Spotify’s engineered features capture artist-level stylistic differences.
...
```{r}
#| fig-cap: "Figure 1: Summary statistics for continuous Spotify audio features."
...
```{r}
#| fig-cap: "Figure 2: Density plots illustrating distribution shapes and scale differences across Spotify audio features."
library(ggplot2)
library(tidyr)
...
```{r}
#| fig-cap: "Figure 3: Correlation heatmap showing relationships among key audio features."
# Correlation Heatmap
...
```{r}
#| fig-cap: "Figure 4: Proportion of Variance Explained by Principal Components"
...
pca_variance_table <- data.frame(
  ...
  Cumulative_Variance = round(cumsum(var_explained), 4)
)
...
Note that we standardized features here to regulate the data and help combat skewness.

```{r}
#| fig-cap: "Figure 5: PCA loading values showing feature contributions to principal components."
#PCA Loadings
...
```{r}
#| fig-cap: "Figure 6: Scree Plot for Proportion of Variance Explained."
...
```{r}
#| fig-cap: "Figure 7: 3D PCA scatter plot illustrating high-dimensional structure among popular songs."
library(plotly)
...
```{r}
#| fig-cap: "Figure 8: Visual loadings plot for PC1 and PC2. Arrows represent feature influence directions."
...
```{r}
#| fig-cap: "Figure 9: PCA projection (PC1 vs. PC2) colored by popularity. Note this also represents our general plot of PC1 vs PC2."
...
```{r}
#| fig-cap: "Figure 10: PCA projection (PC1 vs. PC2) colored by genre."
...
```{r}
#| fig-cap: "Figure 11: Elbow plot used to select an appropriate number of clusters for k-means."
...
```{r}
#| fig-cap: "Figure 12: K-means clustering results visualized in PCA space (PC1 vs. PC2)."
library(ggplot2)
...
```{r}
#| fig-cap: "Figure 13: Cluster-level feature profiles showing mean audio characteristics for each cluster."

kable(
  cluster_feature_means,
  digits = 3,
  caption = "Mean Audio Feature Values by K-means Cluster (Popular Songs)"
)
```

```{r}
#| fig-cap: "Figure 14: A PCA Comparison of Two known artists: Taylor Swift vs. Drake"
#| 
artist_pca <- pca_scores %>%
  mutate(artist = spotify$artists) %>%
  filter(
    grepl("Taylor Swift", artist) | grepl("Drake", artist)
  ) %>%
  mutate(
    artist = ifelse(grepl("Taylor Swift", artist), "Taylor Swift", "Drake")
  )

# Plot PCA comparison
ggplot(artist_pca, aes(x = PC1, y = PC2, color = artist)) +
  geom_point(alpha = 0.6, size = 2) +
  theme_minimal(base_size = 12) +
  labs(
    title = "PCA Comparison of Two Artists: Taylor Swift vs. Drake",
    x = "PC1",
    y = "PC2",
    color = "Artist"
  )
...
The resulting projections in figure 14 show that songs by the Drake and Taylor Swift tend to occupy different regions of PCA space, with Drake’s tracks aligning more strongly with components associated with intensity and speech-related features (more toward the top right of the plot), and Taylor Swift’s songs exhibiting greater variation along dimensions related to melody and acoustic structure (more toward the bottom left of the plot). This case study illustrates how unsupervised dimensionality reduction can be used to compare musical identities using only quantitative audio descriptors, which is truly interesting as these identities are typically compared qualitatively.

Although we initially considered comparing songs across musical decades, we ultimately did not pursue this analysis due to concerns about the reliability of the data from older music and the lack of reliable release-date information in the dataset.
